...
\begin{equation*}
    \begin{aligned}
        \displaystyle 0 &\leq y^2_{k} \leq \lVert \pmb{\text{y}} \rVert^{2} =  \sum^{n}_{k = 0} \frac{y^2_i}{\lVert y \rVert^{2} } \lVert \pmb{\text{y}} \rVert^{2} \implies 0 \leq \rho(\pmb{\text{y}})_{k} \lVert \pmb{\text{y}} \rVert^{2} \leq \lVert \pmb{\text{y}} \rVert^{2} \implies 0 \leq \rho(\pmb{\text{y}})_{k} \leq 1
    \end{aligned}    
\end{equation*}

\begin{equation*} 
    \begin{aligned}
        \displaystyle\lVert \pmb{\text{y}} \rVert^{2} &= \sum^{n}_{k = 0} y^2_{k} =  \sum^{n}_{k = 0} \frac{y^2_i}{\lVert y \rVert^{2} } \lVert \pmb{\text{y}} \rVert^{2} = \sum^{n}_{k = 0} \rho(\pmb{\text{y}})_{k} \lVert \pmb{\text{y}} \rVert^{2} \implies \sum^{n}_{k = 0} \rho(\pmb{\text{y}})_{k} = 1.  \ \ \qedsymbol{}
    \end{aligned}    
\end{equation*}
