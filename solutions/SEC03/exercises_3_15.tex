...
\end{proof}
\subsection*{3.15.9}
Note that the Reyni entropy can be expressed as follows: \ $\text{H}_\alpha(\text{p}(X)) = \frac{\ln \mathbb{E} \left[(p(X))^{\alpha-1}\right]}{1 - \alpha} = \frac{\ln\displaystyle\int_{\tiny \text{sup}(X)}(p(x))^{\alpha-1} d P}{\alpha - 1}$. In last expression $\frac{d P}{d x} = p(x)$; This representation is a consecuence of the \href{https://en.wikipedia.org/wiki/Radon%E2%80%93Nikodym_theorem}{Radon-Nikodym's Theorem}. Now, let's analyse the following function definied as a parametric integral. Let $I(\alpha)$ definied as:\\
$I(\alpha) = \displaystyle\int_{\tiny \text{sup}(X)}(p(x))^{\alpha-1} d P$\newline 
\begin{proof}
    \\
    \underline{CASE $0 < p(x) < 1$}:\\
    \\
    Let $\{\alpha_n\}_{n \in \mathbb{N}} \subset \mathbb{R}$ a monotonicaly decreasing secuence of reals tending to 1. With this we can construct an increasing sequence of P-integrable functions of the form $\{(p(x))^{\alpha_n -1}\}_{n \in \mathbb{N}}$. Then, by 
    construction the point limit will be the function $f(x) = 1$ which is P-integrable. Applying the \href{https://proofwiki.org/wiki/Monotone_Convergence_Theorem_(Measure_Theory)}{Lebegue's monotone convergece theorem}:\\
    ...
    \\
    \underline{CASE $p(x) > 1$}:\\
    \\
    The same is true in this case. It is worth to note that if the sequence $\{\alpha_n\}_{n \in \mathbb{N}}$ is choosen to be decreasing then the sequence of functions is increasing, the contrary situation still holds.\\
    \\
    Bacause the function $f(\alpha,x)$ is derivable the above can be then used to compute the limit $\lim_{\alpha \to 1 }\text{H}_\alpha(\text{p}(X))$. By a simple application $\alpha \to 1$ we find an indetermination of the type $0/0$. Using L'Hôpital's rule we get:

    $\lim_{\alpha \to 1 }\text{H}_\alpha(\text{p}(X)) = \lim_{\alpha \to 1}\frac{1}{-1}\frac{1}{\displaystyle\int_{\tiny \text{sup}(X)}(p(x))^{\alpha-1} d P}$
\end{proof}    
\end{document}
