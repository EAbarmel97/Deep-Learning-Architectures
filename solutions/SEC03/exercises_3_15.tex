...
\begin{proof} 
    $\displaystyle \int p(x) q(x) d x  = \displaystyle \int \left| \ p(x) q(x) \ \right| d x \leq  {\lVert p \lVert}^{2}_{2} {\lVert q \lVert}^{2}_{2} \implies 0 < \displaystyle\frac{\displaystyle \int p(x) q(x) d x}{{\lVert p \lVert}^{2}_{2} {\lVert q \lVert}^{2}_{2}} \leq 1$. Taking logarithms to both sides in the inequality we get and 
    multiplying by $-1$:\\
